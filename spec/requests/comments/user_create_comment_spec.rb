...
describe 'Usuário cria comentário', type: :request do
  ...
  it 'com parâmetros incorretos' do
    ...
    post event_post_comments_path(event_id: event.event_id, post_id: post.id, locale: :'pt-BR'), params: { comment: { content: '' } }

    expect(flash[:alert]).to eq 'Falha ao salvar o comentário'
  end
end
