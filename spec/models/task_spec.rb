...
RSpec.describe Task, type: :model do
  describe '#initialize' do
    it 'cria uma tarefa com os atributos corretos' do
      task = Task.new(code: 'T1', title: 'Tarefa 1', description: 'Descrição da tarefa', certificate_requirement: 'Opcional', attached_contents: [])
  ...
  describe '#need_certificate' do
    it 'retorna true se a tarefa for obrigatória para certificado' do
      task = Task.new(code: 'T2', title: 'Tarefa 2', description: 'Descrição', certificate_requirement: 'Obrigatória')
      expect(task.need_certificate).to be true
    end

    it 'retorna false se a tarefa não for obrigatória' do
      task = Task.new(code: 'T3', title: 'Tarefa 3', description: 'Descrição', certificate_requirement: 'Opcional')
